...
# Change the WebCam Id as per requirement
...
while True:
    ...
    if time.time() - prev_time >= Frame_interval:
        prev_time = time.time()
        detect = frame.copy()
        detections = model.predict(frame, confidence=modelConfidence, overlap=modelOverlap).json()
