...
modelTsign = YOLO("models/door.pt")
...
while True :
    ...
    tSignRes2 = modelTsign(frame,stream=True)
    ...
    for reS in tSignRes2:
        tSboxes = reS.boxes
        for box in tSboxes :
            detected.append(str(modelTsign.names[int(box.cls[0])]))
            x1, y1, x2, y2 = box.xyxy[0]  # bounding box coordinates
            x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
            frame = cornerRect(frame, x1, y1, x2, y2, str(modelTsign.names[int(box.cls[0])]))
